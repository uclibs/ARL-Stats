...
string_agg(bibloc.location_code, ' ') AS Location
 ...
 -- Edit bib format here
    JOIN sierra_view.bib_record_location bibloc ON (i.id = bibloc.bib_record_id AND (i.bcode2 = 'a' OR i.bcode2 = 't')) -- Bib Format mono
    -- JOIN sierra_view.bib_record_location bibloc ON (i.id = bibloc.bib_record_id AND (i.bcode2 = 's')) -- Bib Format serials
    -- JOIN sierra_view.bib_record_location bibloc ON (i.id = bibloc.bib_record_id AND (i.bcode2 = 'c' OR i.bcode2 = 'd')) -- Bib format score
    -- JOIN sierra_view.bib_record_location bibloc ON (i.id = bibloc.bib_record_id AND (i.bcode2 = 'e' OR i.bcode2 = 'f')) -- Bib format map
    -- JOIN sierra_view.bib_record_location bibloc ON (i.id = bibloc.bib_record_id AND (i.bcode2 = 'i' OR i.bcode2 = 'j')) -- Bib format audio
    -- JOIN sierra_view.bib_record_location bibloc ON (i.id = bibloc.bib_record_id AND i.bcode2 = 'g') -- Bib format projected
    -- JOIN sierra_view.bib_record_location bibloc ON (i.id = bibloc.bib_record_id AND i.bcode2 = 'k') -- Bib format graphic
    -- JOIN sierra_view.bib_record_location bibloc ON (i.id = bibloc.bib_record_id AND i.bcode2 = 'm') -- Bib format eresource
    -- JOIN sierra_view.bib_record_location bibloc ON (i.id = bibloc.bib_record_id AND i.bcode2 = 'p') -- Bib format mixed
 ...
 -- Formats that require single pass
    AND i.record_num >= '1000000'

 -- Special Collections BUAR set3 format 'p' is problematic, two passes, use: 
 ...
 -- Formats that require multiple passes (e.g., monographs) patterns
    ...
    -- AND i.record_num >= '4000001' -- pass 3
    -- AND i.record_num <  '5000000' 
    -- AND i.record_num >= '5000001' -- pass 4
