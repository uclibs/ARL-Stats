...
Question 1, set 2, Winkler 2017-18
...
--uncomment lines for different formats

    JOIN sierra_view.bib_record_location bibloc ON (i.id = bibloc.bib_record_id AND (i.bcode2 = 'i' OR i.bcode2 = 'j')) --audio
    -- JOIN sierra_view.bib_record_location bibloc ON (i.id = bibloc.bib_record_id AND i.bcode2 = 'm') --eresource
    -- JOIN sierra_view.bib_record_location bibloc ON (i.id = bibloc.bib_record_id AND i.bcode2 = 'k') --graphic
    -- JOIN sierra_view.bib_record_location bibloc ON (i.id = bibloc.bib_record_id AND (i.bcode2 = 'a' OR i.bcode2 = 't')) --mono
    -- JOIN sierra_view.bib_record_location bibloc ON (i.id = bibloc.bib_record_id AND i.bcode2 = 'g') --projected
    -- JOIN sierra_view.bib_record_location bibloc ON (i.id = bibloc.bib_record_id AND i.bcode2 = 'r') --realia
    -- JOIN sierra_view.bib_record_location bibloc ON (i.id = bibloc.bib_record_id AND (i.bcode2 = 's')) --serials
    
    -- JOIN sierra_view.bib_record_location bibloc ON (i.id = bibloc.bib_record_id AND (i.bcode2 = 'h' OR i.bcode2 = 'v')) --micro
    -- JOIN sierra_view.bib_record_location bibloc ON (i.id = bibloc.bib_record_id AND i.bcode2 = 'p') --mixed (no data to export)
   ...
   -- Many formats will require only 1 pass
    ...
    AND (item.location_code LIKE 'hy%'
      OR holdingloc.location_code LIKE 'hy%'
      )
